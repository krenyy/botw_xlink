...
class XLNK:
    ...
    def frombinary(cls, data: bytes):
        ...
        inst.resourceAssetParamTable = ParamTable.read(
            br, inst.header.triggerOverwriteParamTablePos - br.tell()
        )
        inst.triggerOverwriteParamTable = ParamTable.read(
            br, inst.header.localPropertyNameRefTablePos - br.tell()
        )

        inst.localPropertyNameRefTable = RefTable.read(
            br, inst.header.numLocalPropertyNameRefTable
        )
        inst.localPropertyEnumNameRefTable = RefTable.read(
            br, inst.header.numLocalPropertyEnumNameRefTable
        )

        inst.directValueTable = DirectValueTable.read(
            br, inst.header.numDirectValueTable
        )
        ...
        inst.exRegion = ExRegion.read(
            br, inst.header.conditionTablePos - inst.header.exRegionPos
        )

        inst.conditionTable = ConditionTable.read(
            br, inst.header.nameTablePos - br.tell()
        )
